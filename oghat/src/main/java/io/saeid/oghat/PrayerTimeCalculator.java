...
public class PrayerTimeCalculator {
    ...
    private double JDate; // Julian day
    // ------------------------------------------------------------
    ...
    public PrayerTimeCalculator() {
        ...
        methodParams.put(CalculationType.JAFARI, Jvalues);

        double[] Kvalues = {18, 1, 0, 0, 18};
        methodParams.put(CalculationType.KARACHI, Kvalues);

        double[] Ivalues = {15, 1, 0, 0, 15};
        methodParams.put(CalculationType.ISNA, Ivalues);

        double[] MWvalues = {18, 1, 0, 0, 17};
        methodParams.put(CalculationType.MWL, MWvalues);

        double[] MKvalues = {18.5, 1, 0, 1, 90};
        methodParams.put(CalculationType.MAKKAH, MKvalues);

        double[] Evalues = {19.5, 1, 0, 0, 17.5};
        methodParams.put(CalculationType.EGYPT, Evalues);

        double[] Tvalues = {17.7, 0, 4.5, 0, 14};
        methodParams.put(CalculationType.TEHRAN, Tvalues);

        double[] Cvalues = {18, 1, 0, 0, 17};
        methodParams.put(CalculationType.CUSTOM, Cvalues);
    ...
    // ---------------------- Time-Zone Functions -----------------------
    // compute local time-zone for a specific day
    ...
    // detect daylight saving in a given day
    ...
    // calculate julian day from a calendar day
    ...
    // convert a calendar day to julian day (second method)
    ...
    // -------------------- Interface Functions --------------------
    // return prayer times for a given day
    ...
    private void setCustomParams(double[] params) {
        ...
        for (int i = 0; i < 5; i++) {
            if (params[i] == -1) {
                ...
                methodParams.put(CalculationType.CUSTOM, params);
            } else {
                methodParams.get(CalculationType.CUSTOM)[i] = params[i];
            }
        }
        this.setCalcMethod(CalculationType.CUSTOM);
    }
    ...
    // compute prayer times at given julian day
